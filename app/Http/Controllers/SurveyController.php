    ...
    public function index(Request $request)
{
    if ($request->ajax()) {
        ...
        $surveyResults = SurveyResponse::with('user', 'question.category')
            ->select('survey_responses.id', 'survey_responses.user_id', 'survey_responses.score', 'questions.category_id', 'users.name as respondent_name')
            ...
            ->addColumn('action', function ($row) {
                $btn = '<div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="' . route('dashboard.survey.result.show', $row['user_id']) . '">
                                    <i class="bx bxs-user-detail me-1"></i> Detail
                                </a>
                                <a class="dropdown-item" href="' . route('dashboard.survey.result.destroy', $row['user_id']) . '"
    ...
    {
        $categories = Category::with(['questions.responses' => function ($query) {
    ...
    public function create()
    {
        ...
        abort_if($alreadySubmitted, 403, 'Anda sudah mengisi survey.');

        $categories = Category::with('questions')->get();
        return view('dashboard.survey.create', compact('categories'));
    }
    ...
    public function store(Request $request) {
        ...
        foreach ($validated['responses'] as $question_id => $score) {
            SurveyResponse::updateOrCreate(
                ['user_id' => auth()->id(), 'question_id' => $question_id],
                ['score' => $score]
            );
        }
    ...
    public function thankyou() {
        return view('dashboard.survey.thankyou');
    }
